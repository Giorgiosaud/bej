...
$(window).on("load", function () {
    ...
    var firstAnimation = new TimelineMax(),
        ...
        objetivo3 = $('.Flex__imageContainer--vertical');
        maxWidth = $('.Flex__imageContainer--vertical img').width();
        firstAnimation.insert(TweenMax.fromTo(cochinoContenedor, 3, {
        height: startHeight,
        opacity: 0
    }, {height: heightCochino, opacity: 1}))
        .insert(TweenMax.from(elementTextRight, 3, {'margin-top': 0}))
        .add(TweenMax.from(objetive2, 1, {opacity: 0}))
            .add(TweenMax.fromTo(objetivo, 1, {width: 1}, {width: maxWidth}));
